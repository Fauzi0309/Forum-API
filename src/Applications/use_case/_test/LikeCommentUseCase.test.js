...
describe("LikeCommentUseCase", () => {
  it("should orchestrating the like comment action correctly with condition not yet liked", async () => {
    ...
    mockUserCommentLikeRepository.addCommentLike = jest
      .fn()
      .mockImplementation(() => Promise.resolve());
    ...
    expect(mockUserCommentLikeRepository.checkCommentLike).toBeCalledWith(
      useCaseParam.commentId,
      useCaseAuth.id
    );
    expect(mockUserCommentLikeRepository.addCommentLike).toBeCalledWith(
      useCaseParam.commentId,
      useCaseAuth.id
    );
  });
  it("should orchestrating the like comment action correctly with condition already liked", async () => {
    ...
    mockUserCommentLikeRepository.deleteCommentLike = jest
      .fn()
      .mockImplementation(() => Promise.resolve());
    // Create the use case instace
    const likeCommentUseCase = new LikeCommentUseCase({
    ...
    expect(mockUserCommentLikeRepository.deleteCommentLike).toBeCalledWith(
      useCaseParam.commentId,
      useCaseAuth.id
    );
  });
